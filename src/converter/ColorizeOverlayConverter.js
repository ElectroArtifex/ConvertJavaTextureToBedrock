			...
			for (const [overlay, color, delete_overlay] of overlays) {
				if (await this.output.exists(overlay)) {
					...
					if (Array.isArray(color) && color.length >= 3) {
						...
						image_overlay.scan(0, 0, image_overlay.getWidth(), image_overlay.getHeight(), (x, y, idx) => {
							image_overlay.bitmap.data[idx] = (image_overlay.bitmap.data[idx] / 255 * color[0]);
							image_overlay.bitmap.data[idx + 1] = (image_overlay.bitmap.data[idx + 1] / 255 * color[1]);
							image_overlay.bitmap.data[idx + 2] = (image_overlay.bitmap.data[idx + 2] / 255 * color[2]);
						});
					}
